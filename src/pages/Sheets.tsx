...
export default function About() {
  ...
  function parseCustomString(input: string): Record<string, string> {
    ...
    while (i < input.length) {
      // --- Parse KEY ---
      // Keys are never quoted, read until ':'
      const colonIdx = input.indexOf(":", i);
      if (colonIdx === -1) break;
      const key = input.slice(i, colonIdx).trim();
      i = colonIdx + 1; // move past ':'
      // --- Parse VALUE ---
      let value: string;
      if (input[i] === '"') {
        ...
        value = input.slice(i + 1, closeQuote); // strip quotes
        i = closeQuote + 1;
        // Consume the trailing comma after closing quote, if present
        if (input[i] === ",") i++;
      } else {
        // Unquoted value — read until next ',' that is followed by a valid "key:" pattern
        // Strategy: scan for ', ' boundaries where the next segment contains a ':'
        let end = i;
        while (end < input.length) {
          const comma = input.indexOf(",", end);
          if (comma === -1) {
            end = input.length;
            break;
          }
          // Check if what follows looks like a new key (text then ':')
          const nextColon = input.indexOf(":", comma + 1);
          const nextComma = input.indexOf(",", comma + 1);
          const isNextKey =
            nextColon !== -1 &&
            (nextComma === -1 || nextColon < nextComma) &&
            /^[a-zA-Z]/.test(input[comma + 1]);
        ...
        i = end + 1; // skip the comma separator
      }
    ...
    fileInput.onchange = (e) => {
      ...
      if (file) {
        ...
        reader.onload = async (event) => {
          ...
          // split into individual lines and trim whitespace
          const lines = fileContent
            .split(/[\r\n]+/)
            .map((line) => line.trim())
            .filter(Boolean);
          lines.shift(); // remove header line
          // parse each line into an object and extract the sort values
          const sorts: string[][] = lines.map(parseCustomString).map((obj) => {
            const randomId = obj.randomId?.trim() ?? "";
            const partId = obj.partId?.trim() ?? "";
            const userCode = obj.urlUserCode?.trim() ?? "";
            const sortValues =
              obj.sort
      ...
      <main className="w-full h-full overflow-hidden max-w-6xl bg-white/60 backdrop-blur-md rounded-2xl shadow-xl p-8 md:p-12">
        <header className="flex items-center gap-6">
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800">
            {t("Google Sheets Data Conversion")}
          </h1>
